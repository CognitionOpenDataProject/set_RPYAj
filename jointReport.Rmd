...
```{r}
rs.aov <- aov(rt ~ Posture * Direction + Error(Subject / (Posture * Direction)), data = df.corrected)

rs.aov.summary <- summary(rs.aov)

compareValues( # compare F value
  reportedValue = 32.65,
  obtainedValue = rs.aov.summary$`Error: Subject:Direction`[[1]]["Direction","F value"]
)

# eyeball p value, is it below .001 as stated?
rs.aov.summary$`Error: Subject:Direction`[[1]]["Direction","Pr(>F)"]
```
...
```{r}
rs.aov.digits <- aov(rt ~ Number + Error(Subject/Number), data=df.corrected)

rs.aov.digits.summary <- summary(rs.aov.digits)

compareValues( # compare F values
  reportedValue = 26.23,
  obtainedValue = rs.aov.digits.summary$`Error: Subject:Number`[[1]]["Number","F value"]
)

# eyeball p value, is it below .001 as stated?
rs.aov.digits.summary$`Error: Subject:Number`[[1]]["Number","Pr(>F)"]
```
